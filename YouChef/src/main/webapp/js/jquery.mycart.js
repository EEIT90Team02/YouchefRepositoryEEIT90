  ...
  var loadMyCartEvent = function(userOptions){
    ...
    if(!$("#" + idCartModal).length) {
      $('body').append(
        ...
        '<button type="button" class="btn btn-primary ' + classCheckoutCart + '">確認送出</button>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '</div>'
    ...
    var drawTable = function(){
      ...
      $.each(products, function(){
        ...
        $cartTable.append(
          ...
          '<td title="Total" class="' + classProductTotal + '">新台幣 ' + total + '</td>' +
        ...
        $cartTable.append(products.length ?
          ...
          '<td></td>' +
          '</tr>'
          : '<div class="alert alert-danger" role="alert" id="' + idEmptyCartMessage + '">您的購物車是空的</div>'
        );
    ...
    */
    ...
    $(document).on('click', "." + classProductRemove, function(){
      ...
      count = 0 ; // 移除時 將點擊次數歸零
      $(".btn-info").text("可購買數量: "+ (maxNum)); //移除時 將原本可購買數量復原
      var cart_button = $('.my-cart-btn');
      //var inputNum = $('input.my-product-quantity');
      cart_button.prop("disabled", false);  //移除時 將原本add to cart 復原
      //inputNum.prop("disabled", false);  //移除時 將原本購物車的input number復原
  ...
  var MyCart = function (target, userOptions) {
    ...
    */
    $target.click(function(){
      ...
      options.clickOnAddToCart($target);//新增的動畫效果 
      ...
      ProductManager.setProduct(id, name, summary, price, quantity, image); //新增右上角購物車數量
      $cartBadge.text(ProductManager.getTotalQuantity());
    });
