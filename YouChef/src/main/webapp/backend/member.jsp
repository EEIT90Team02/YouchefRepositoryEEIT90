                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12 gutter">
                            <div class="sales">
								<table class="table table-hover table-curved">
										...
										<tbody>
										<c:forEach var="element" items="${memberAll}">		
												   <tr class="warning"> 
												   		<td>${element.m_id}</td>
												   	<c:choose>
												   		<c:when test="${element.ac_status==0}">
												   			<td>一般會員</td>
												   		</c:when>
												   		<c:when test="${element.ac_status==1 }">
												   			<td>會員大廚</td>
												   		</c:when>
												   		<c:when test="${element.ac_status==3 }">
												   			<td>停權</td>
												   		</c:when>
												   	</c:choose>	
														<td>${element.email}</td>
													...
													<c:choose>
														<c:when test="${element.ac_status == 3 }">
															<td><a href="<c:url value="/backend/changeMemStatus.controller?action=resume&m_id=${element.m_id }"/>"  
																   class="btn btn-success"
																   onClick="return(confirm('確認回覆該會員權利嗎?'))">復權
																</a>
															</td>
														</c:when>
														<c:otherwise>
															<td><a href="<c:url value="/backend/changeMemStatus.controller?action=suspend&m_id=${element.m_id }"/>"  
																   class="btn btn-danger"
																   onClick="return(confirm('你確定要停權該會員嗎?'))">停權
																</a>
															</td>
														</c:otherwise>
													</c:choose>
